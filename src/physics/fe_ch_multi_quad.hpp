...
struct FE_CH_MultiQuad {
    ...
    template <int D>
    std::vector<Field<D>> compute_mu(const FieldStore<D>& /*S*/,
        ...
        std::vector<Field<D>> mu;
        mu.reserve(N);
        for (int i = 0; i < N; ++i) mu.emplace_back(phi[i]->g);
    ...
    template <int D>
    double bulk_density_at(const std::vector<const Field<D>*>& phi, int p) const {
        const int N = (int)phi.size();
        double s = 0.0;
        for (int i = 0; i < N; ++i) {
